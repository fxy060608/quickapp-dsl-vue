...
module.exports = function quickAppFactory(exports, document, quickappHelper) {
  ...
  var isRuntimeComponent = makeMap('richtext,trisition,trisition-group', true)
  ...
  var platformModules = [attrs, klass, events, style, domProps, transition]
  ...
  Vue$2.prototype.$mount = function(el, hydrating) {
    ...
    var component = mountComponent(this, el && query.bind(this)(el, this.$document), hydrating)
    // 将页面与根VM链接
    if (type === 'page') {
  ...
  Vue$2.prototype._initExternalData = function() {
    ...
    console.trace(
      '### App Framework ### 页面VM中声明的权限定义：' + JSON.stringify(this.$options._descriptor)
    )
      ...
      if (matchFromExPackage || matchFromInPackage) {
        ...
        console.trace(
          '### App Framework ### 传递外部数据' +
            extName +
            '，原值为:' +
            JSON.stringify(this$1._data[extName])
        )
        console.trace(
          '### App Framework ### 传递外部数据' +
            extName +
            '，新值为:' +
            JSON.stringify(externalData[extName])
        )
  ...
  Vue$2.prototype._mergeAccess2Data = function(options) {
    ...
    if (
      ...
      throw new Error(
        '页面VM对象中的属性data不可与' + accessors.join(',') + '同时存在，请使用private替换data名称'
      )
